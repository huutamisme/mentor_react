...
export default function MockInterview({ experience, skills, pricing, activeTab }) {
    // Hàm lọc dữ liệu dựa trên các props
    const filteredCards = MockInterviewData.filter((card) => {
                            ...
                            >
                                ...
                                <div className="card-body flex items-center">
                                    ...
                                    <div className="card-actions justify-end">
                                        <NavLink
                                            to={`/services/mock/${card.id}?tab=${activeTab}`}
                                            className="py-2 px-4 bg-customBlue text-background rounded-full text-sm font-bold">
                                            Chi tiết
                                        </NavLink>
                                        <NavLink to={`/services/book/mock/${card.id}?tab=${activeTab}`} className="py-2 px-4 bg-customBlue text-background rounded-full text-sm font-bold">
                                            Đặt lịch hẹn ngay
                                        </NavLink>
                                    </div>
                                </div>
