...
export default function ProjectAdvise({ experience, skills, pricing, activeTab }) {
    // Hàm lọc dữ liệu dựa trên các props
    const filteredCards = ProjectAdviseData.filter((card) => {
                            ...
                            >
                                ...
                                <div className="card-body flex items-center">
                                    ...
                                    <div className="card-actions justify-end">
                                        <NavLink to={`/services/project/${card.id}?tab=${activeTab}`} className="py-2 px-4 bg-customBlue text-background rounded-full text-sm font-bold">
                                            Chi tiết
                                        </NavLink>
                                        <NavLink to={`/services/book/project/${card.id}?tab=${activeTab}`} className="py-2 px-4 bg-customBlue text-background rounded-full text-sm font-bold">
                                            Đặt lịch hẹn ngay
                                        </NavLink>
                                    </div>
                                </div>
